...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($pricerequests as $pricerequest) {
            ...
            if ($pricerequest->region == '1') {
                $requestRegion = 'Tallinn';
            }
            if ($pricerequest->region == '2') {
                $requestRegion = 'Tartu';
            }
            if ($pricerequest->region == '3') {
                $requestRegion = 'Pärnu';
            }
...
// WHEN ADMIN SENDS A PRICE REQUEST, IT GOES TO CLIENT'S EMAIL
    public function requestsPost(Request $request)
    {
        ...
        $dataInsert = ['request_id' => $request->id, 'answered_by' => Auth::user()->id, 'answer' => $request->answer];
